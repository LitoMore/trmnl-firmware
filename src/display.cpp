...
static bool display_update_epaper(int refreshMode, bool wait, bool writePlane = false, uint8_t plane = PLANE_0)
{
...
#else
    if (writePlane) {
        Log_info("EPD writePlane start: plane=%d busy=%d millis=%lu", plane, digitalRead(EPD_BUSY_PIN), millis());
        int rc = bbep.writePlane(plane);
        Log_info("EPD writePlane end: rc=%d busy=%d millis=%lu", rc, digitalRead(EPD_BUSY_PIN), millis());
        if (rc != BBEP_SUCCESS) {
            Log_error("EPD writePlane failed: %d", rc);
            return false;
        }
    }
    ...
    Log_info("EPD refresh start: mode=%d wait=%d busy=%d millis=%lu", refreshMode, wait,
             digitalRead(EPD_BUSY_PIN), millis());
    int rc = bbep.refresh(refreshMode, wait);
    Log_info("EPD refresh end: rc=%d busy=%d millis=%lu", rc, digitalRead(EPD_BUSY_PIN), millis());
    if (rc != BBEP_SUCCESS) {
        Log_error("EPD refresh failed: %d", rc);
        return false;
    }
